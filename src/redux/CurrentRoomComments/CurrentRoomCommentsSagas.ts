...
import { ADD_LIKE, REMOVE_LIKE, SET_ROOM_COMMENTS } from './Types';
...
function* workerAddLikeSaga({ payload }: AnyAction) {
  if (payload.uid) {
    yield call(
      addLike, payload.commentID, payload.uid
    );

    const comments: Promise<Array<CommentType> | FirebaseError> = yield call(getRoomComments, payload.roomID);
...
function* workerRemoveLikeSaga({ payload }: AnyAction) {
  if (payload.uid) {
    yield call(
      removeLike, payload.commentID, payload.uid
    );
...
function* watchAddLikeSaga(): Generator<
  ForkEffect<never>,
  void,
  unknown
> {
...
function* watchRemoveLikeSaga(): Generator<
  ForkEffect<never>,
  void,
  unknown
> {
...
export { watchAddLikeSaga, watchRemoveLikeSaga };
